...
html_files = [f for f in os.listdir('.') if f.endswith('.html') and 'old_index' not in f]
for f in html_files:
...
if '5: \'Technology' not in js:
    # It wasn't updated yet, because it previously crashed before main.js update or it was done? Wait, I didn't successfully write it before it crashed on nav update. Let's fix main.js.
    js_old_real = """    const segmentDescriptions = {
    ...
    js = js.replace(js_old_real, js_new)
